...
export const listLastestProducts = () => async (dispatch) => {
  try {
    dispatch({ type: PRODUCT_LASTEST_LIST_REQUEST });

    const { data } = await axios.get(`/api/products/home`);

    dispatch({
      type: PRODUCT_LASTEST_LIST_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: PRODUCT_LASTEST_LIST_FAIL,
      payload:
